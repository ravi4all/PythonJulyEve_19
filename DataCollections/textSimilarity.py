...
#1. Tokenization
t1 = text_1.lower().split()
t2 = text_2.lower().split()
...
for i in range(len(c)):
    for j in range(len(words_1)):
        if words_1[j].endswith(c[i]):
            words_1[j] = words_1[j].replace(c[i],'')

for i in range(len(c)):
    for j in range(len(words_2)):
        if words_2[j].endswith(c[i]):
            words_2[j] = words_2[j].replace(c[i],'')
